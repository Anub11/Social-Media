...
function insertPost()
{
	if (isset($_POST['sub'])) 
	{
		...
		if (strlen($content) > 25) {
			echo "<script>alert('Please Use less 25 character');</script>";
			echo "<script>window.open('home.php', '_self')</script>";
		}
		else {
			if (strlen($upload_image) >= 1 && strlen($content) >= 1) {

				move_uploaded_file($image_temp, "$random_number$upload_image");
				...
				if ($run) {
					...
					echo "<script> alert('Your Post Updated a movement ago!');</script>" ;
					echo "<script> window.open('home.php', '_self');</script>";


					$updated = "update users set posts='yes' where user_id='$user_id'";
...
function get_posts(){
	...
	while ($row_posts = mysqli_fetch_array($run_post)) {
		...
		// now displaying data from database 
		 		...
		 		<div class='panel panel-info'>
		 			<div class='panel-body'>
		 				<form action='' method='post' class='form-inline'>
		 					<textarea placeholder='place your comment here!' class='pb-cmnt-textarea' name='comment'>
		 					</textarea>
